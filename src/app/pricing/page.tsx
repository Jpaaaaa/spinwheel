...
function Check({ included }: { included: boolean }) {
  return included
    ...
    : <IconMinus className="mx-auto h-5 w-5 text-slate-300" />;
}
    ...
    <div className="safe-px mx-auto max-w-5xl px-4 py-12 sm:px-6 sm:py-16 md:py-20">
      ...
      <div className="mt-12 grid gap-6 sm:grid-cols-2">
        <div className="glass p-6 sm:p-8">
          ...
          <p className="mt-2 text-sm text-slate-600">Perfect for small events and teams</p>
        ...
        <div className="glass-strong relative border-violet-200/50 bg-gradient-to-br from-violet-600/5 to-blue-600/5 p-6 sm:p-8">
          <span className="absolute -top-3 right-6 rounded-full border border-white/40 bg-violet-600 px-3 py-1 text-xs font-semibold text-white shadow-md">Popular</span>
          <h2 className="text-lg font-semibold text-slate-800">Pro</h2>
          <div className="mt-4 flex items-baseline gap-1">
            <span className="text-4xl font-bold text-slate-800">€29</span>
            <span className="text-slate-500">/month</span>
          </div>
          <p className="mt-2 text-sm text-slate-600">For professional events and organisations</p>
          <ul className="mt-8 space-y-3">
            {proFeatures.map((f) => (
              <li key={f} className="flex items-center gap-2.5 text-sm text-slate-700">
                <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-500/15">
      ...
      <div className="mt-16">
        <h2 className="text-center text-xl font-bold text-slate-800">Feature comparison</h2>
        <div className="glass mt-8 overflow-x-auto">
          <table className="w-full min-w-[480px] text-sm">
            ...
            <tbody>
              {comparison.map((row) => (
                <tr key={row.feature} className="border-b border-white/20 last:border-0">
